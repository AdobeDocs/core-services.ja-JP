...
* 値の昇順または降順に列を並べ替える。これは、[!UICONTROL Name,] の [!UICONTROL Email,] および [!UICONTROL Last Login] の列に適用されます。
...
[!UICONTROL Users] ページでユーザーの詳細を表示するには、ユーザーの電子メールをクリックします。
...
* 値の昇順または降順に列を並べ替える。これは、[!UICONTROL product profile,] [!UICONTROL Product,] [!UICONTROL Instance,] [!UICONTROL Number of users,] および [!UICONTROL Number of Developers,] Columns に適用されます。
...
製品プロファイルの詳細を CSV ファイルにエクスポートできます。  [!UICONTROL Export CSV] のオプションを選択すると、次の 2 つの CSV ファイルが生成されます。
...
このセクションには、製品プロファイルで使用できる権限とリソースの完全なリストが表示されます。製品プロファイルに含まれている権限およびリソースは、「✔」でマークされています。 権限とリソースのリストは、見やすいようにタブと列に分類されています。タブと列には、現在の製品に適用されるセクションのリストが表示されます。
