...
* [CX Enterprise ID サービス &#x200B;](https://experienceleague.adobe.com/en/docs/id-service/using/intro/overview): ECID サービスは、コレクション ドメインがサイト ドメインと一致しているかどうかにかかわらず、常にファーストパーティ Cookieを設定します。 JavaScriptを使用して、サイトのドメインにCookieを配置します。
...
Adobeは、プライバシーとデータセキュリティに真剣に取り組んでいます。 プライバシー機関、規制当局、AdChoicesなどのプログラムと連携し、個人が自身のデータをどのように使用するのかを制御できます。

Adobe CX EnterpriseのほとんどのCookieは、個人情報を保存しません。 セキュリティが確保され、レポート、コンテンツ、広告などに使用できるのは企業だけです。 Adobeは、業界全体の匿名レポート（デジタルマーケティングInsightレポートなど）を除き、このデータを他のお客様やサードパーティと共有することはありません。
