...
seo-title: コアサービス向けのExperience Cloudソリューションの有効化
...
>[!NOTE]
>
>Targetの場合、mbox. jsからat. jsに [移行](https://marketing.adobe.com/resources/help/en_US/target/ov2/t_target-migrate-atjs.html)します。
...
![](assets/step3_icon.png) Admin Console でのユーザーと製品の管理.
...
ヘルプについては [、Experience Cloudユーザーおよび製品管理](../admin-getting-started/admin-getting-started.md#topic_3FCB4099640647E3B2411ADBFCE81909) を参照してください。
...
1. Adobe ID を持っている。
1. サインイン [!DNL marketing.adobe.com]します。
...
> [!NOTE]
> 
> エンタープライズグループとソリューショングループがマッピングされると、新しいユーザーは自動的にリンクされます。（ソリューションの資格情報が自動的に作成されて Adobe ID にリンクされます）。

以下のセクションでは、実装を最新化する方法を説明します。実装を最新化すると、Experience Cloud のコアサービスが有効になります。

## 手順 2：Dynamic Tag Managerまたは起動を使用したExperience Cloud IDサービスの実装（アドビによる） {#section_3C9F6DF37C654D939625BB4D485E4354}

Experience Cloud コアサービスを有効にする最も簡単な方法は、Dynamic Tag Manager の [Experience Cloud ID サービスツール](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-dtm-implement.html)を使用して、Analytics および Target 用にコアサービスを自動的にアクティブにすることです（または Launch, by Adobe を使用できます）。
...
* 訪問者IDサービス `visitor.setCustomerIDs({"crm_id":"52mc210tr42"})` を使用して呼び出します。または
...
**[Android](https://marketing.adobe.com/resources/help/en_US/mobile/android/?f=methods) および [iOS](https://marketing.adobe.com/resources/help/en_US/mobile/ios/?f=methods) Mobileアプリケーションに追加の顧客IDを設定する方法については、&quot;Experience Cloud IDサービス」の節を参照してください。
...
訪問者 API など JavaScript ライブラリを更新して Analytics の実装を最新化することが推奨されます。これを行う簡単な方法は、Dynamic Tag Managementに [!DNL Adobe Analytics] ツールを追加し、設定方法 *`Automatic`* として指定することです。

Dynamic Tag Managementで、&quot; **[!UICONTROL <Web Property Name>]**/**[!UICONTROL 概要]**/ツールを**[!UICONTROL 追加]**/**[!UICONTROL Adobe Analytics]**をクリックします。導入情報については、Dynamic Tag Management の[Adobe Analytics の設定](https://marketing.adobe.com/resources/help/en_US/dtm/?f=analytics_dtm)を参照してください。

## 手順 5. （Adobe Target）Adobe Target の実装を最新化する{#section_C2F4493C7A36406DAE2266B429A4BD24}

* Dynamic Tag Management で [Adobe Target ツール](https://marketing.adobe.com/resources/help/en_US/dtm/target.html)を追加してライブラリの検索を自動化することが推奨されます。Dynamic Tag Managementで、&quot; **[!UICONTROL <Web Property Name>]**/**[!UICONTROL 概要]**/ツールを**[!UICONTROL 追加]**/**[!UICONTROL Adobe Target]**。** 注意：**Dynamic Tag Management を使用して、Target（およびその他のソリューション）用に Experience Cloud ID サービスをデプロイすることもできます。Target でコアサービスを使用するには、Experience Cloud ID サービスのアップデートが** 必要 **です。
...
* 
   >[!IMPORTANT]
   >
   >すべての解析ユーザーは、顧客属性などコアサービス用に既にプロビジョニングされています。Analytics ユーザーになっていないユーザーがいる場合は、そのユーザーのプロビジョニングをカスタマーケアに依頼します。

## 手順 6. コアサービスの実装を確認する {#section_E641782A0F4F44AF8C9C91216BE330D5}

Experience Cloud ID サービスがサイトに正しく実装されていることを確認するには、次の手順を実行します。

1. Experience Cloud ID サービスに対するリクエストを確認できるように、サイトの Cookie を消去します（リクエストは最初の訪問時におこなわれ、その後は訪問者あたり約 1 週間に 1 回おこなわれます）。1. パケットアナライザーまたはWebブラウザーデバッガーのネットワークパネルを使用して、送信先のリクエストを探し [!DNL dpm.demdex.net]ます。
1. 応答に次の値と `d_mid` 値が含まれていることを確認します。 `_setMarketingCloudFields({"d_mid":"4235...`
...
## 手順 7. ユーザーおよび製品の管理 {#section_B6E95F4E0E12483CB9DA99CBC0C5A4AF}

起動して実行したら **[!UICONTROL 、管理]** / **[!UICONTROL 管理コンソールの起動]** に移動して、ユーザーおよび製品プロファイルを管理します。
...
詳しく [は、顧客属性を参照してください](../attributes/attributes.md#concept_ACFEE7C8B8E94875BA0825CDF4913AF1)
