...
>[!NOTE]
> 以前は、Adobeでは、HTTPS 用と HTTP 用の 2 つの CNAME を設定することをお勧めしていました。 トラフィックを暗号化することをお勧めします。また、ほとんどのブラウザーでは HTTP を強くお勧めしません。そのため、HTTP に CNAME を設定することはお勧めしません。 CNAME を HTTP 用に設定する必要がある場合は、カスタマーケアにお問い合わせください。Adobe
