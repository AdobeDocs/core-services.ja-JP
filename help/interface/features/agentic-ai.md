...
description: CX Enterpriseのさまざまなアプリケーションで、Agentic AIをどこで利用できるかをご確認ください。
...
Adobe Experience Platform Agentsは、[Experience Platform Agent Orchestrator](https://experienceleague.adobe.com/ja/docs/experience-cloud-ai/experience-cloud-ai/home)によって強化され、CX Enterprise アプリケーション内でエージェント型AI機能を有効にします。

これらのエージェントは、タスクの自動化、インサイトの迅速な提供、ワークフローの合理化に役立ちます。 その結果、作業の効率を高め、CX Enterpriseからより多くの価値を引き出すことができます。

CX EnterpriseのAI エージェントには、次のいずれかの方法でアクセスできます。

* [既存のCX Enterprise アプリケーション](#existing-apps)
* [AIを活用したCX Enterprise製品](#ai-first-apps)

以下の節では、CX Enterpriseでエージェンティック AIを有効にするための2つの方法について説明します。
...
エージェントは、次のいずれかのカテゴリのお客様向けに、既存のCX Enterprise アプリケーションで有効にできます。
...
### 既存のCX Enterprise アプリのAI エージェント {#existing-apps-table}

次の表に、既存のCX Enterprise アプリケーションで使用可能なExperience Platform Agentsの一覧を示します。
