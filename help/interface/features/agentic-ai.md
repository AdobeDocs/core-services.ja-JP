...
既存のアプリケーションでは、自然言語を使用して、[AIアシスタント](https://experienceleague.adobe.com/en/docs/experience-cloud-ai/experience-cloud-ai/home)の会話型インターフェイスを介してAdobe Experience Platform Agentに指示を与えることができます。 AIアシスタントは、フルスクリーン表示と右側のレール表示の両方で使用できます。

お客様の既存のExperience Cloudアプリでは、次のいずれかのカテゴリで担当者を有効にできます。

* Adobe Experience Platform担当者AIクレジットライセンスを購入しています
* 使用量ベースの体験版に含まれます（AIクレジットは限定的で提供）
* Agent OrchestratorプロモーションSKU （期限付き体験版ライセンス）をトランザクションしました

AIエージェントを使用して&#x200B;_エージェントジョブ_&#x200B;を実行すると、AIクレジットが消費されます。 エージェントのジョブとAIクレジットに関する詳細については、[エージェントのジョブとAIクレジットの消費](https://experienceleague.adobe.com/en/docs/core-services/interface/features/ai-credit-consumption)を参照してください。

AI担当者は、_お客様の_&#x200B;の入力と監督に従い、製品レベルのアクセス制御を尊重します。 ジョブを実行したり、基本のExperience Cloudアプリケーションで使用する権限を持つデータにアクセスしたりすることしかできません。

### 既存のExperience CloudアプリのAIエージェント {#existing-apps-table}

次の表に、既存のExperience Cloudアプリケーションで使用できるExperience Platformエージェントを示します。

| 担当者名 | 機能 | サポート対象アプリケーション |
| --- | ---------- | ---------- |
| [オーディエンスエージェント](https://experienceleague.adobe.com/ja/docs/experience-cloud-ai/experience-cloud-ai/agents/audience) | 自然言語プロンプトを使用してチームがオーディエンスを作成、管理、最適化して、より簡単に、効率よく、迅速に市場に投入できるようにします。 | <ul><li>Real-Time CDP（B2B、B2C、B2Pエディション）</li><li>Adobe Journey Optimizer（B2BおよびB2Cエディション）</li></ul> |
| [コンテンツアドバイザーエージェント](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/agents/content-advisor/overview) | <ul><li>自然言語を使用して、チームが企業全体で最も関連性の高いコンテンツを迅速に見つけることができるため、検索にかかる時間が短縮され、迅速な意思決定と実行が可能になります。</li><li>自然言語プロンプトを使用して、ソースアセットからビジュアルコンテンツバリアントを簡単に作成できます。</li></ul> | <ul><li>Adobe Experience Manager Assets</li></ul><ul><li>Dynamic Media(Cloud Service)</li></ul> |
| [データインサイトエージェント](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-overview/cja-b2c-overview/data-analysis-ai) | データに関する質問にすばやく回答します。 データビューのコンポーネントと実際のデータを使用して、Analysis Workspace で関連するビジュアライゼーションを作成します。 | <ul><li>Customer Journey Analytics（B2BおよびB2Cエディション）</li></ul> |
...
| [Journey Agent](https://experienceleague.adobe.com/en/docs/experience-cloud-ai/experience-cloud-ai/agents/ajo-agent) | チームがマルチタッチのカスタマージャーニーを大規模にすばやく作成、分析、最適化できるようにします。 | <ul><li>Adobe Journey Optimizer（B2B と B2C のエディション）</li></ul> |
| [ 製品サポート担当者 ](https://experienceleague.adobe.com/en/docs/experience-cloud-ai/experience-cloud-ai/agents/product-support) | ワークフローを離れることなくサポートの問題をトラブルシューティングしたり、カスタマーサポートチケットを作成したり、AI アシスタントを使用してケースの進行状況を追跡したりできます。 | <ul><li>Real-Time CDP（B2B、B2C、B2P の各エディション）</li><li>Adobe Journey Optimizer（B2B と B2C のエディション）</li><li>Customer Journey Analytics（B2B と B2C のエディション）</li><li>Adobe Experience Manager</li></ul> |
...
| --- | ---------- | ---------- |
| [Experimentation Agent](https://experienceleague.adobe.com/en/docs/experimentation-accelerator/using/get-started/experiment-accelerator-security) | インサイトの自動化、分析、統合を行い、影響の大きい実験や成長の機会を一元的なワークスペースからすばやく特定できます。しかも、手動プロセスを削減できます。 | <ul><li>AJO Experimentation Accelerator</li></ul> |
...
| [Product Advisor Agent](https://experienceleague.adobe.com/en/docs/brand-concierge/content/documentation/overview) | 個々の好みや行動に合わせてカスタマイズされた、インテリジェントでコンテキストに対応した製品検出により、コンバージョンとエンゲージメントを強化します。 | <ul><li>Adobe Brand Concierge</li></ul> |
...
* [AEMのエージェントの概要](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/ai-in-aem/agents/overview)
