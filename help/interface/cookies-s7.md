...
* 現在のページや画像、現在のズームレベルなど、ユーザーのセッション状態を追跡します。
...
* ユーザーがeCatalogを他のユーザーと共有するために電子メールを開始すると、2番目のAS2ビューアブレットのノート注釈コンテンツがAdobeのサーバーにコピーされ、受信者に提供できるようになります。 受信者がビューアセッションを開始すると、ノート注釈コンテンツがサーバーから取得されて cookie にコピーされます。この機能はほとんど使用されないので、有効期限は設定されず、古いコンテンツも削除されません。現在、サーバー上で無期限に保持されます。
