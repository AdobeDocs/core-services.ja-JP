...
組織 ID を表示するには、Experience Cloud ランディングページに移動するか、（![](assets/menu-icon.png)）を選択してから、「**[!UICONTROL 管理]**」を選択します。組織 ID は、「[!UICONTROL Experience Cloud の概要]」ページまたは「[!UICONTROL 管理]」ページの下部にあります。
