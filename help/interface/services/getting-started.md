---
description: クロスアプリケーションサービス用 Adobe Analytics および Adobe Target のアプリケーションを最新化します。 CX Enterpriseのサービス利用方法をご覧ください。
...
既存のお客様は、アプリケーション実装を最新化し、CX Enterpriseを実装できます。 これにより、Adobe Analytics、Audience Manager、Adobe Target全体で顧客属性とオーディエンス機能を使用できます。

## 管理者としてログイン {#admin-sign-in}

管理者になると、[experience.adobe.com](https://experience.adobe.com) でログインできます。

**[!UICONTROL Admin Console]** リンクは、ユーザーと製品ライセンスを管理するためのCX Enterprise メニューナビゲーションで使用できます。
...
これらのグループをCX Enterprise エンタープライズグループにマッピングする場合、それらのユーザーはアプリケーションアカウントの資格情報を手動でAdobe IDにリンクする必要があります。

CX Enterpriseの[Link アカウントを参照](https://experienceleague.adobe.com/ja/docs/core-services/interface/administration/organizations)

>[!NOTE]
>
>エンタープライズグループとソリューショングループのマッピング後、新しいユーザーは自動的にリンクされます。 （ソリューションの資格情報が自動的に作成されて Adobe ID にリンクされます）。

以下の節では、実装を最新化する方法を説明します。 導入を最新化することで、CX Enterpriseのコアサービスが可能になります。

## ユーザーとしてログイン {#user-sign-in}

CX Enterpriseにログインするには、次の操作が必要です。
...
## CX EnterpriseのAdobe AnalyticsとAdobe Targetの要件 {#experience-cloud-requirements}

CX Enterpriseを使用するための[!DNL Analytics]および[!DNL Adobe Target]要件：
...
## [!UICONTROL CX Enterprise ID Service] の実装
...
CX Enterprise コアサービスを有効にする最も簡単な方法は、[!UICONTROL Experience Platform Launch]の[CX Enterprise ID サービス拡張機能](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html?lang=ja)を介して、AnalyticsとAdobe Targetに対して自動的に有効にすることです。

>[!NOTE]
>
>CX Enterprise ID サービスの完全なヘルプ（以前は訪問者ID）については、[CX Enterprise ID サービスの概要](https://experienceleague.adobe.com/docs/id-service/using/intro/overview.html?lang=ja#intro)を参照してください。
...
| タスク | 説明 |
| --- | --- |
| [Analytics用CX Enterprise ID サービスの実装](https://experienceleague.adobe.com/ja/docs/analytics/implementation/id/overview) | また、追加の[顧客 ID](https://experienceleague.adobe.com/ja/docs/id-service/using/reference/authenticated-state) を設定することを推奨します。 これらのIDは各訪問者に関連付けられ、CX Enterpriseの現在および将来の機能を有効にします。 |
...
訪問者が web サイトで認証をおこなうとき、または別の方法で本人確認をおこなうとき、実装では、その人物の CRM 顧客 ID をページまたはアプリに公開する必要があります。 その後、適切な関数呼び出しを使用して、お客様IDをCX Enterpriseに同期できます。 この同期により、訪問者のCRM顧客IDがCX Enterpriseに保存され、その顧客の属性がCX Enterpriseで使用できるようになります。
...
[CX Enterprise](https://experienceleague.adobe.com/docs/mobile-services/android/overview.html?lang=ja)および[Android](https://experienceleague.adobe.com/docs/mobile-services/ios/overview.html?lang=ja) モバイルアプリケーションで追加の顧客IDを設定する方法について™構文の例については、*iOS ID サービス*&#x200B;の節を参照してください。
...
履歴データを有効にするには、サポートにお問い合わせください。

## レポートスイートをCX Enterprise組織にマッピングする
...
ファーストパーティ Cookieを使用している場合は、データ収集CNAMEとクロスドメイントラッキングについて、[CNAMEとCX Enterprise ID サービス &#x200B;](https://experienceleague.adobe.com/docs/id-service/using/reference/analytics-reference/cname.html?lang=ja)を参照してください。
...
* ライブラリの取得が自動的に行われるように、[Adobe Target拡張機能](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/target-v2/overview.html?lang=ja)を[!UICONTROL Experience Platform] タグに追加することをお勧めします。 [!UICONTROL Experience Platform] タグを使用して、Adobe Target（およびその他のアプリケーション）用に[CX Enterprise ID サービス拡張機能](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html?lang=ja)を設定することもできます。 Adobe TargetでPeople サービスを使用するには、[!UICONTROL CX Enterprise ID Service]の更新&#x200B;**が必要です**。
* [!UICONTROL Experience Platform]個のタグを使用していない場合は、[mbox ライブラリを手動で更新します](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/implement-target-for-client-side-web.html?lang=ja)。
...
1. サイトのCookieをクリアして、CX Enterprise ID サービスへのリクエストを確認できるようにします（リクエストは初回訪問時に発生し、その後は週に1人の訪問者ごとに発生します）。
...
CX Enterprise IDを含む応答が必要です：

![CX Enterprise ID](../assets/mac_id_response.png)を含む応答が必要です

CX Enterprise ID （`mid`または&#x200B;_訪問者ID_&#x200B;とも呼ばれます）を含むAnalytics イメージリクエスト：

CX Enterprise ID![&#128279;](../assets/mid.png)を含むAnalytics イメージリクエスト

mbox リクエストのCX Enterprise ID:

![mbox リクエスト内のCX Enterprise ID](../assets/mbox_request.png)

### 猶予期間とは

CX Enterprise ID サービスをデプロイすると、新しい訪問者はデータ収集サーバーからAnalytics CX Enterprise IDを受け取らなくなります。 サイトのセクションがまだID サービスを実装していない場合、訪問者がこれらのセクションを参照すると、CX Enterprise IDは認識されず、訪問者には従来のAnalytics訪問者IDが割り当てられます。 その結果、訪問者数が重複してカウントされたり、誤った属性が割り当てられたりするなどの問題が生じる可能性があります。

例えば、サイトのサポートセクションを別の CMS で管理している場合は、そのセクション用に別の Analytics JavaScript ファイルを使用していることがあります。 ID サービスをサポートサイトにデプロイする前にメインサイトにCX Enterprise IDをデプロイした場合、新規訪問者はサポートセクションにアクセスすると、従来のAnalytics IDを受け取ります。 両方のサイトセクションにまたがる訪問は、異なる訪問として報告されます。

複数のJavaScript ファイルまたはその他のテクノロジー（Flashなど）を使用しているサイトにCX Enterprise ID サービスをデプロイすると、連携に問題が発生する可能性があります。 これらの問題は、サイトのすべての部分で同時にCX Enterprise ID サービスを有効にする必要があるため発生します。 猶予期間を設定することで、新規訪問者はID サービスから引き続きAnalytics訪問者IDを受け取ることができます。 訪問者は、訪問者ID サービスを使用するようにアップグレードされていないサイトのセクションで一貫して識別できます。
